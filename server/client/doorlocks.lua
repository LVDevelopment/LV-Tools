...
---@return string
local function BuildEntry(d)
    local nameLine
    if d.objName and d.objName ~= '' then
        nameLine = ("\t\tobjName = '%s',"):format(d.objName)
    else
        nameLine = ("\t\tobjHash = %s,"):format(d.objHash or 0)
    end

    local jobParts = {}
    for _, j in ipairs(d.authorizedJobs or {}) do
        jobParts[#jobParts + 1] = ("'%s'"):format(j)
    end
    local jobsStr = '{ ' .. table.concat(jobParts, ', ') .. ' }'
    ...
    return table.concat({
        ...
        ("\t\tauthorizedJobs = %s,"):format(jobsStr),
